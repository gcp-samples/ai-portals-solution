...
try {
  ...
  console.log('Running transformProductDetail unit tests...');

  // Test Case 16: Validate transformProductDetail for REST product with mock list of products
  var mockProductsList = JSON.parse(fs.readFileSync(path.join(__dirname, 'mock-products.json'), 'utf8'));

  var mockDeployments = {
    deployments: [
      {
        name: "projects/apigee-klab1/locations/europe-west1/deployments/qwen-dep",
        endpoints: [
          { uri: "https://custom-endpoint.com/qwen" }
        ],
        apiVersions: [
          "projects/apigee-klab1/locations/europe-west1/apis/b2499a30-1db8-4231-9255-30e5dee044f7/versions/d5a04e32-874a-4e77-afe1-e7b4f46f3311"
        ]
      }
    ]
  };
  ...
  var detailResult = transformProductDetail(mockDeployments, mockVersionSpec, "ai-vertexqwen-1", mockProductsList);
  assert.ok(detailResult, 'Detail result should be defined');
  assert.strictEqual(detailResult.id, 'ai-vertexqwen-1', 'ID should match');
  assert.strictEqual(detailResult.endpoint, 'https://custom-endpoint.com/qwen', 'Endpoint should be extracted from matching deployment');
  assert.strictEqual(detailResult.spec, 'openapi: 3.0.0\ninfo:\n  title: Qwen', 'Spec should be extracted from contents');
  assert.strictEqual(detailResult.displayStyle, 'REST', 'DisplayStyle should be REST');

  // Test Case 6: Validate base64 decoding and JSON parsing in transformProductDetail
  var base64MockSpec = {
    contents: "eyJhIjogMX0=", // Base64 for {"a": 1}
    mimeType: "application/json"
  };
  var detailResultB64 = transformProductDetail(mockDeployments, base64MockSpec, "ai-vertexqwen-1", mockProductsList);
  assert.deepStrictEqual(detailResultB64.spec, { a: 1 }, 'Spec should be decoded and parsed as JSON object');
